...
function LiveTicker() {
  ...
  useEffect(() => {
    async function loadTickerData() {
      try {
        ...
        const activeRivalry =
          rivalryDataDb && rivalryDataDb.length > 0
            ? rivalryDataDb[0]
            : fallbackRivalrySeasons.find((s) => s.status === "active") ||
        ...
        const nextMatch =
          matchData && matchData.length > 0 ? matchData[0] : null;
        ...
        if (nextMatch) {
          const mDate = new Date(nextMatch.match_date).toLocaleDateString(
            "en-IN",
            {
              weekday: "short",
              month: "short",
        ...
        if (activeRivalry) {
          items.push(
            `🏆 RIVALRY UPDATE: ${activeRivalry.title} active! Series score: Mavericks (${activeRivalry.mavericks_main_wins}) - (${activeRivalry.neurostrikers_main_wins}) NeuroStrikers`,
          );
        }

        // Add articles excerpts
        sourceArticles.forEach(
          (art: {
            title: string;
            excerpt?: string;
            ...
            const categoryText = art.category
              ? `[${art.category.toUpperCase()}]`
              : "NEWS";

            if (excerptText) {
              items.push(
                `📰 ${categoryText}: ${articleTitle} — "${excerptText}"`,
              );
            } else {
        ...
        const totalMatches = rivalrySeasons
          ? rivalrySeasons.reduce(
              (sum: number, s: any) => sum + (s.total_matches_played || 0),
              0,
            )
          : 0;
  ...
  return (
    <div
      ...
      style={{
        background:
          "linear-gradient(90deg, #0C1E30 0%, #07111F 50%, #0C1E30 100%)",
      }}
    >
      {/* Breaking tag */}
...
> = {
  "Match Report": {
    bg: "bg-jcc-accent/15 border-jcc-accent/30",
    text: "text-jcc-accent",
    dot: "bg-jcc-accent",
  },
  Analysis: {
    bg: "bg-jcc-green/15 border-jcc-green/30",
    text: "text-jcc-green",
    dot: "bg-jcc-green",
  },
  "Origin Story": {
    bg: "bg-jcc-gold/15 border-jcc-gold/30",
    text: "text-jcc-gold",
    dot: "bg-jcc-gold",
  },
  Culture: {
...
function CategoryBadge({ category }: { category: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-[8px] font-black uppercase tracking-widest ${style.bg} ${style.text}`}
    >
    ...
    >
      <Link href={`/chewvana-times/${article.slug}`} className="block">
        <div
          ...
          style={{
            background: "linear-gradient(160deg, #1a0a0a 0%, #07111F 100%)",
          }}
        >
          {/* Top editorial rule */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-0">
            ...
            <div className="flex flex-col justify-center p-8 sm:p-10">
              ...
              <h3
                ...
                style={{
                  fontFamily: "var(--font-fraunces, var(--font-heading))",
                }}
              >
                {article.title}
              </h3>
    ...
    >
      <Link href={`/chewvana-times/${article.slug}`} className="block h-full">
        <div
          ...
          style={{
            background: "linear-gradient(160deg, #0C1E30 0%, #07111F 100%)",
          }}
        >
          {/* Image */}
...
function SkeletonCard({ large = false }: { large?: boolean }) {
  return (
    <div
      className={`rounded-2xl border border-white/[0.06] animate-pulse overflow-hidden ${large ? "h-[340px]" : "h-[380px]"}`}
      style={{
        background: "linear-gradient(160deg, #0C1E30 0%, #07111F 100%)",
      }}
    />
...
export default function ChewvanaTimesSection() {
  ...
  return (
    <section
      ...
      style={{
        background:
          "linear-gradient(180deg, #07111F 0%, #0C1E30 50%, #07111F 100%)",
      }}
    >
      {/* Top divider */}
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          ...
          >
            ...
            <p className="text-white/40 text-base font-medium max-w-2xl leading-relaxed">
              Unfiltered match reports, tactical dispatches, and editorial
              investigations from inside the circle.
            </p>
          </motion.div>
        </div>
